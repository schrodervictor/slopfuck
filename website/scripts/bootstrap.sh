...
# Bootstrap script for the slopfuck marketing site.
...
#
# Example:
#   ./scripts/bootstrap.sh slopfuck-website 012345-6789AB-CDEF01 us-central1
...
bootstrap() {
    ...
    msg "" \
        ...
        "     $ make tf-apply" \
        "" \
        "  3. After first apply, copy the ACME challenge TXT value from" \
        "     the Firebase Hosting console (or Terraform output) into" \
        "     infra/tf/dns.tf, then re-apply to provision SSL." \
        "" \
        "  4. Build and deploy:" \
        "     $ make deploy"
}
